...
class LooseCouple:
    def __init__(self, imu_params, imu_data, gnss_data, init_vel, init_euler, save=None):
        ...
        self.ins = INS(imu_data, gnss_data, init_vel, init_euler, dt=0.004)
    ...
    def run(self):
        ...
        gnss_index, imu_index = 0, 0
        while gnss_data[gnss_index][0] <= self.imu_data[imu_index][0]:
            gnss_index += 1

        while imu_index < len(self.imu_data):
            # ins状态更新
            self.ins.step()
            ...
            Phi = np.eye(STATUS_DIMENSION) + F * ins.dt
            ...
            # ② 若此时有 GNSS 数据则进行更新
            # =====================================
            if gnss_index < len(gnss_data) and gnss_data[gnss_index][0] <= self.imu_data[imu_index][0]:
                gnss = gnss_data[gnss_index][1:4]
                # -----------------------
                # GNSS 位置观测
                # -----------------------
                lb = np.zeros(3)
                ...
                z_vel = ins.vel - gnss_data[gnss_index][4:7]
                y = z_vel - H_vel @ x
...
if __name__ == "__main__":
    ...
    timestamp_imu, gyro, acc = read_sensor_combined(path)
    timestamp_gps, gps, eph, epv, vel = read_gps_data(path)
    timestamp_mag, mag = read_mag_data(path) # 没用到吧
    timestamp_estimator, quat, vel_kf, position, covariances = read_estimator_results(path)
    start_time = max(
        timestamp_imu[0],
        timestamp_gps[0],
        timestamp_mag[0],
    ...
    def cut_by_start(ts, *arrays):
        """裁掉 ts < start_time 的数据，同时对其它数组做相同裁剪"""
        idx = np.searchsorted(ts, start_time, side='left')
        return (ts[idx:],) + tuple(a[idx:] for a in arrays)


    # 对各传感器按 start_time 裁剪
    timestamp_imu, gyro, acc = cut_by_start(timestamp_imu, gyro, acc)
    timestamp_gps, gps, eph, epv, vel = cut_by_start(timestamp_gps, gps, eph, epv, vel)
    timestamp_mag, mag = cut_by_start(timestamp_mag, mag)
    timestamp_estimator, quat, vel_kf, position, covariances = cut_by_start(
        timestamp_estimator, quat, vel_kf, position, covariances
    )
    init_cnb = attitude.quat2dcm(quat[0][:])
    init_vel_b = init_cnb.T @ vel[0][:]
    init_euler = attitude.quat2euler(quat[0][:], 'zyx')

    save = ['P']
    loose = LooseCouple(imu_params, np.concatenate((timestamp_imu.reshape(-1, 1),acc,gyro), axis=1), np.concatenate((timestamp_gps.reshape(-1, 1), gps, vel), axis=1), init_vel_b, init_euler, save=[])
    ...
    gnss_len = 10
    ins_len = int(min((len(acc) / len(gps) * gnss_len, len(acc))))



    plt.plot(ekf_pos[:ins_len, 1], ekf_pos[:ins_len, 0], label="EKF", linewidth=2, alpha=0.5)
    plt.plot(ins_result[:ins_len, 1], ins_result[:ins_len, 0], label="INS", linewidth=2, alpha=0.8)
    plt.plot(gps[:gnss_len, 1], gps[:gnss_len, 0], label="GPS", linestyle='--', linewidth=1.8, alpha=1)
    # plt.plot(position[:, 1], position[:, 0], label="Reference", linewidth=1.5)
